  ...
  const staticExtensions = useMemo(() => [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'autoUpdateContactInfo', name: 'Auto-update Contacts', desc: 'Automatically refresh contact details from your phone.', icon: <ContactIcon /> }
      ]
    }
  ], []);
        ...
        <div className="main-content" id="main-content">
          ...
          {activePanel === 'extensions' && (
            <div className="pulselink-panel">
              ...
              {/* Tabs */}
              <div className="line-tabs" style={{ marginBottom: 20, overflowX: 'auto', flexWrap: 'nowrap' }}>
                {['All', ...staticExtensions.map(g => g.title), 'Developer'].map(cat => (
                  <button
                    key={cat}
                    className={`chip ${extensionCategory === cat ? 'active' : ''}`}
              ...
              {/* Quick Setup (Only on 'All' tab and no search) */}
              {extensionCategory === 'All' && !extensionSearch && (
                <div className="settings-card" style={{marginBottom: 20}}>
                  <h4>Quick Setup</h4>
              ...
              {extensionCategory !== 'Developer' && (
                <div className="extension-list">
                  {staticExtensions.map(group => {
                    if (extensionCategory !== 'All' && group.title !== extensionCategory) return null;

                    const filteredItems = group.items.filter(item => {
                        const term = extensionSearch.toLowerCase();
                    ...
                    return (
                      <div key={group.title} className="extension-category" style={{marginBottom: 32}}>
                        {(extensionCategory === 'All' || extensionSearch) && <h4 style={{marginBottom: 16, color: 'var(--ink)'}}>{group.title}</h4>}
                        <div className="home-grid">
