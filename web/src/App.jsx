...
// Custom comparator for ThreadItem to prevent unnecessary re-renders
// Compares: isActive, showPreviews, and core thread properties (id, address, snippet)
// Note: onSelect is intentionally excluded as it's a function reference that may change
const areThreadsEqual = (prev, next) => {
  // Defensive check for null/undefined thread objects
  if (!prev.thread || !next.thread) return prev.thread === next.thread;
...
// Custom comparator for MessageItem to prevent unnecessary re-renders
// Compares: showPreviews and core message properties (id, body, date, type)
const areMessagesEqual = (prev, next) => {
  // Defensive check for null/undefined message objects
  if (!prev.msg || !next.msg) return prev.msg === next.msg;
