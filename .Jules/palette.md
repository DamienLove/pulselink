...
## 2024-05-29 - Hex Color Inputs for Theming
**Learning:** Native `input[type="color"]` pickers are great for visual selection but frustrate power users who want to copy/paste specific hex codes from design tools. They often lack a text input fallback in many browsers.
**Action:** Created a composite `<ColorInput />` component that pairs the visual picker with a synchronized text input, catering to both casual and power users while maintaining accessibility.
